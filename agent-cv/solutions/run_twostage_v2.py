"""NorgesGruppen Object Detection — Two-Stage (YOLO + DINOv2) v2.
...
GALLERY_FILE = "gallery_combined.npy"
GALLERY_LABELS_FILE = "gallery_combined_labels.json"
...
def classify_crops_batched(dino_session, dino_input_name, crops_bgr, valid_mask,
                           gallery, gallery_labels, yolo_labels, top_k=5,
                           batch_size=16):
    """Classify crops using batched DINOv2 inference + kNN against gallery.

    Skips invalid crops (valid_mask=False) and falls back to YOLO label for those.
    """
    n = len(crops_bgr)
    ...
    # Collect valid crop indices and preprocess
    ...
    similarities = embeddings @ gallery.T  # (V, G)
...
def main():
    ...
    for img_path in image_files:
        ...
        dino_labels = classify_crops_batched(
            dino_sess, dino_input, crops, valid_mask, gallery, gallery_labels,
            yolo_labels, DINO_TOP_K, DINO_BATCH_SIZE)
