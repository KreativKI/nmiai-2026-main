"""NorgesGruppen Object Detection -- Two-Stage with PRE-TRAINED Classifier.
...
Stage 2: DINOv2 ViT-S + pre-computed PCA whitening + pre-computed LinearSVC
         Trained offline on 22,731 crops (vs 9,949 at runtime).
         Zero runtime training cost (saves ~49s).
...
DINO_BATCH_SIZE = 16
PCA_DIMS = 320
CROP_PAD_RATIO = 0.0  # 0.0 = match gallery training distribution; 0.1 = add context

# Pre-computed classifier files (trained on GCP with ALL 22,731 crops)
...
def classify_crops_pretrained(dino_session, dino_input_name, crops_bgr, valid_mask,
    ...
    # Confidence from decision function (sigmoid of max margin)
...
def main():
    ...
    print(f"Pre-trained classifier loaded: {len(svc_classes)} classes, "
          f"coef shape {svc_coef.shape}")
