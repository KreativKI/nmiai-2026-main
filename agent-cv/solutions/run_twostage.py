"""NorgesGruppen Object Detection — Two-Stage (YOLO + DINOv2).

Stage 1: YOLO11m detects product bounding boxes (detection mAP, 70% of score)
Stage 2: DINOv2 ViT-S reclassifies each crop via kNN against product gallery
         (classification mAP, 30% of score)

Detection scores stay from YOLO (preserves detection ranking).
Category IDs come from DINOv2 (improves classification on rare products).
...
def classify_crops(dino_session, dino_input_name, crops_bgr, gallery, gallery_labels,
    ...
    For each crop, embed with DINOv2, find top-K nearest gallery entries,
    vote by similarity weight. Falls back to YOLO label if gallery has no
    coverage for the crop's neighborhood.
    """
    if len(crops_bgr) == 0:
    ...
    for i in range(len(embeddings)):
        ...
        # Weighted vote: accumulate similarity per class
        class_votes = {}
        ...
        if class_votes:
            ...
            result_labels.append(winner)
        else:
            # Fallback to YOLO if gallery is empty (shouldn't happen)
            result_labels.append(int(yolo_labels[i]))

    return np.array(result_labels, dtype=np.int32)
...
def main():
    ...
    for img_path in image_files:
        ...
        # Stage 2: Crop each detection, classify with DINOv2
        crops = []
        for box in boxes:
        ...
        dino_labels = classify_crops(
            dino_sess, dino_input, crops, gallery, gallery_labels,
            yolo_labels, DINO_TOP_K)

        for i, (box, score) in enumerate(zip(boxes, det_scores)):
            x1, y1, x2, y2 = box
            cat_id = int(dino_labels[i]) if i < len(dino_labels) else int(yolo_labels[i])
