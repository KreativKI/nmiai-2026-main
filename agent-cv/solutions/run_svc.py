"""NorgesGruppen Object Detection — Two-Stage with LinearSVC Classifier.

Stage 1: YOLO11m detects product bounding boxes (detection mAP, 70% of score)
Stage 2: DINOv2 ViT-S + PCA whitening (320 dims) + LinearSVC trained at runtime
         on 9,949 gallery embeddings (classification mAP, 30% of score)

LinearSVC learns proper decision boundaries vs kNN voting. Validated: +1.2%
classification accuracy over kNN k=10 dist² (0.8460 vs 0.8340).
...
def classify_crops_svc(dino_session, dino_input_name, crops_bgr, valid_mask,
                       classifier, yolo_labels, pca_mean, pca_matrix,
                       batch_size=16):
    """Classify crops using batched DINOv2 + PCA whitening + LinearSVC."""
    n = len(crops_bgr)
    ...
    # LinearSVC prediction + decision function for confidence
    predicted = classifier.predict(embeddings_pca)
    decision = classifier.decision_function(embeddings_pca)

    # For multiclass, decision is (n_samples, n_classes). Take max per sample.
    if decision.ndim == 2:
        cls_confidence = decision.max(axis=1)
    else:
        cls_confidence = decision

    # Sigmoid to map decision distance to [0, 1]
...
def main():
    ...
    # Load gallery, PCA whiten, train classifier
    ...
    classifier = LinearSVC(C=1.0, max_iter=1000, dual=False, tol=1e-3)
    classifier.fit(gallery_pca, gallery_labels)
    ...
    for img_path in image_files:
        ...
        # Stage 2: Crop, embed, PCA whiten, SVC classify
        crops = []
        valid_mask = []
        for box in boxes:
            x1 = max(0, int(box[0]))
        ...
        dino_labels, cls_conf = classify_crops_svc(
            dino_sess, dino_input, crops, valid_mask,
            classifier, yolo_labels, pca_mean, pca_matrix, DINO_BATCH_SIZE)
