"""NorgesGruppen Object Detection — Two-Stage with Centroid Classifier.

Stage 1: YOLO11m detects product bounding boxes (detection mAP, 70% of score)
Stage 2: DINOv2 ViT-S + PCA whitening (384 dims) + nearest centroid classifier
         reclassifies each crop (classification mAP, 30% of score)

Centroid classifier: average all gallery embeddings per category into one
centroid vector. Classify by nearest centroid. Absorbs crop-level noise,
420x faster than kNN, +2-5% accuracy over kNN on validation.
...
def build_centroids(gallery_pca, gallery_labels):
    """Compute per-category centroid from PCA-whitened gallery embeddings."""
...
def classify_crops_centroid(dino_session, dino_input_name, crops_bgr, valid_mask,
                            centroids, centroid_labels, yolo_labels,
                            pca_mean, pca_matrix, batch_size=16):
    """Classify crops using batched DINOv2 + PCA whitening + nearest centroid."""
    n = len(crops_bgr)
    ...
    # Nearest centroid classification (single matrix multiply)
    similarities = embeddings_pca @ centroids.T  # (V, num_categories)
    best_idx = np.argmax(similarities, axis=1)

    for vi, orig_i in enumerate(valid_indices):
        result_labels[orig_i] = int(centroid_labels[best_idx[vi]])
...
def main():
    ...
    # Load gallery, PCA whiten, build centroids
    ...
    centroids, centroid_labels = build_centroids(gallery_pca, gallery_labels)
    ...
    for img_path in image_files:
        ...
        # Stage 2: Crop with padding, embed, PCA whiten, centroid classify
        crops = []
        valid_mask = []
        for box in boxes:
            bx1, by1, bx2, by2 = box
        ...
        dino_labels = classify_crops_centroid(
            dino_sess, dino_input, crops, valid_mask,
            centroids, centroid_labels, yolo_labels,
            pca_mean, pca_matrix, DINO_BATCH_SIZE)
