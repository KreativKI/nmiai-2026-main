...
CONF_THRESHOLD = 0.05
IOU_THRESHOLD = 0.5
TOP_K = 3                    # Consider top-K gallery matches
...
def main():
    ...
    for img_path in image_files:
        ...
        # NMS using YOLO labels (for detection dedup)
        ...
        # Combine: use YOLO bbox + detection score, but DINOv2 category
        # Blend scores: detection confidence * classification confidence
        for i, (box, det_score) in enumerate(zip(boxes, det_scores)):
            x1, y1, x2, y2 = box

            if len(dino_labels) > i:
                cat_id = int(dino_labels[i])
                # Blend: sqrt(det_score * dino_similarity) gives balanced score
